...
const Grid_withRecolt = () => {
  ...
  const options = ["MINE_DE_CHARBON", "ATELIER_DE_CHARBONIER", "MOULIN", "EXCAVATRICE_A_FER"];
  ...
  const baseUrlConst = "http://51.210.117.22:8080/equipes/593a8cc1-bbec-4c4d-a3af-e926c1324e33/villageois";
  ...
  const sendActionRequest = async () => {
    ...
    const actionUrl = `${baseUrlConst}/${selectedCheckboxId}/demander-action`;
    ...
    try {
      const response = await fetch(actionUrl, {
        ...
        body: JSON.stringify({ action: "CONSTRUIRE", reference: selectedValue }),
      });

      if (response.ok) {
        alert("Action demandée avec succès !");
      } else {
        alert("Échec de la demande d'action.");
      }
    } catch (error) {
      console.error("Erreur lors de la demande d'action :", error);
    }
  };
  ...
  const moveVillager = async (direction) => {
    ...
    if (villager) {
      ...
      let newY = villager.positionY;
        ...
        if (direction === "haut") newY = Math.min(gridSize - 1, newY + 1);  // La direction "haute" diminue en Y, mais ici on l'inverse
  ...
  const handleRecolter = async () => {
    if (selectedCheckboxId && quantite > 0 && type !== "") {
      ...
      const url = `http://51.210.117.22:8080/equipes/593a8cc1-bbec-4c4d-a3af-e926c1324e33/villageois/${villager.idVillageois}/demander-action`;
      ...
      const token = "eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICI2VG5aX1o4WlM2YTlYczJqckl4RTZLS0lNbjgyaTdxN3Z4cTRtY3dQOE13In0..jJ-MRMYihh8gI6BU_1J5KLc_VlyUFHTXt5b2T2D3DnlDkMxO62I4S0tDIr60EqLe_xRI3HwAFTVstnfsQMwefaAHpCmuzn-i333a4OFurTb9AK-P6o0Ptlij7GF58gk0l0iyOfn0W001xVwzCEKxmBfuD3u-9yyTdSGXeaGzxPhSkpJGBGwo7fDcRtLyRKr9UWpkCjg42UUFR3os1pdSqmQWY0HZ9OfJ1Wb9RfK2OXxG42f8hHlMHGemguh7pGuWGqXlditEjqBwDdcdQ_DNxjeRytJrgo_0p7kbZmXsb5gteY6aTHNUvBpTFjopPIF_HtlezvqV1449xvpOa_rlsg"; // Token should be securely stored or passed dynamically
    ...
    <div className="grid gap-2 grid-cols-6">
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="mt-4">
          ...
          <button onClick={() => moveVillager("haut")} className="px-4 py-2 m-1 bg-blue-500 text-white rounded">Haute</button>
...
<select
      ...
      <button
          onClick={sendActionRequest}
          className="w-24 h-12 bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 rounded-lg transition duration-200"
        >
          Construire
        </button>
</div>
